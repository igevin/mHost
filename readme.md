...
mHost 的目标不是简单地提供一个 `/etc/hosts` 编辑器，而是把常见的域名解析切换、环境隔离、广告拦截规则管理做成一个更安全、更易用的桌面工具。它优先考虑性能、稳定性和用户体验，尽可能减少后台开销，并支持在无需超级用户授权的情况下使用。
...
- 免超级用户授权：优先采用不直接修改系统 Hosts 文件的方案，降低权限风险。
...
应用优先避免直接修改系统关键文件。对于必须涉及系统网络配置的能力，应提供清晰说明，并尽可能把权限请求控制在最小范围内。
...
mHost 的一个关键目标是在不要求超级用户授权的情况下工作。为实现这一点，应用可以优先采用用户态网络能力，例如本地 DNS、代理解析或应用内解析层，而不是直接修改系统 Hosts 文件。
...
如果部分高级功能在特定系统上必须依赖系统级配置，应用应明确提示原因，并提供可选启用方式，而不是默认要求用户授权。

## 计划支持的平台

| 平台 | 状态 | 说明 |
| --- | --- | --- |
| macOS | 计划支持 | 面向开发者和日常桌面用户 |
| Windows | 计划支持 | 支持开发、测试和广告屏蔽场景 |
| Linux | 可扩展 | 后续可根据需求适配 |
