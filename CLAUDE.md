...
- **SIGINT / SIGTERM** (`src-tauri/src/lib.rs::setup`): cleanup runs with `interactive=false` (no sudo prompt in the dev loop — `pnpm tauri dev` + Ctrl+C should feel instant). System-DNS restore on next launch is covered by the on-disk recovery marker + `try_recover_dns`. Cleanup is bounded by `tokio::time::timeout(700ms)`; once it returns (or times out) the handler calls `std::process::exit(0)` directly, bypassing Tauri's `RunEvent::ExitRequested` path which intermittently hangs on macOS (tao async-exit known issue). A 1500 ms background-thread watchdog `std::process::exit(0)` is spawned *before* cleanup as a last-resort backstop — this is the only change in the contract from the prior `interactive=true` behavior.

All three call `commands::dns::cleanup_dns_on_exit(state, interactive)` (idempotent — `dns_enabled=false` is a no-op). Any new code that may terminate the process must keep this DNS-cleanup guarantee — leaks leave the system pointing at `127.0.0.1` with no way to recover the user's original DNS.
